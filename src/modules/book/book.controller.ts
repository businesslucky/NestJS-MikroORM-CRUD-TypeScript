import { Get, Controller, Post, Put, HttpStatus, HttpException, Param, Body, Delete } from '@nestjs/common';
...
export class BookController {
  ...
  async create(@Body() body: any) {
    if (!body.title ) {
      throw new HttpException('One of `title` is missing', HttpStatus.BAD_REQUEST);
    }

    const book = new Book(body.title);
  ...
  @Delete(':id')
  async delete(@Param() id: string, @Body() body: any) {
    const book = await this.bookRepository.findOne(+id);

    if(!book){
      throw new HttpException('Book not found', HttpStatus.NOT_FOUND);
    }
    return this.bookRepository.nativeDelete(book);
